...
handles.boothStatusDir = 'C:\Users\Lillian\Documents\MATLAB\ARTSYNEW\MonitorNICard';
...
handles.led = [16 15 14 13 12 11 10 9]; % "EFE"
...
ledstatus(5) = boothStatus.led2;
load booth2Status
boothStatus.led = 1;
boothStatus.led2 = 1;
ledstatus(2) = boothStatus.led;
ledstatus(6) = boothStatus.led2;
load booth3Status
boothStatus.led = 1;
boothStatus.led2 = 1;
ledstatus(3) = boothStatus.led;
ledstatus(7) = boothStatus.led2;
load booth4Status
boothStatus.led = 1;
boothStatus.led2 = 1;
ledstatus(4) = boothStatus.led;
ledstatus(8) = boothStatus.led2;
...
for i = 1:length(feederstatus),
    ...
    %cardOut(handles.led(i+4)) = ledstatus(i+4); % "SAI Basilio"
end
pause(0.01)
putvalue(handles.dio.Line([1:8]),cardOut) % "EFE" Changed 8 to 16
...
while monitorState == 1,
    ...
    for i = 1:4,
        ...
        ledstatusb(i+4) = boothStatus.led2; % sai and basilio
    end
    ...
    if ~isempty(feederChange) || ~isempty(lightsChange) || ~isempty(ledChange), % "EFE"
    ...
    if timerState == 1,
        ...
        while (tCurrent<tOn || tCurrent>=tOff) && timerState == 1 && monitorState == 1,
            ...
            if turnOff == 1,
                ...
                % Update the card if the feeder or light status' have changed
                if ~isempty(feederChange) || ~isempty(lightsChange),
                    putvalue(handles.dio.Line([1:8]),cardOut)
                end
