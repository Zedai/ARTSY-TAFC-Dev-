function varargout = GoNoGo(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @GoNoGo_OpeningFcn, ...
    'gui_OutputFcn',  @GoNoGo_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function GoNoGo_OpeningFcn(hObject, eventdata, handles, varargin)
...
if handles.startOrStop == 1 && handles.running == 0,
    ...
    [handles.stims, handles.nGo, handles.nNoGo, handles.blockSize, handles.song, handles.songNames, handles.bigBlockSize, handles.reinforceSchedule]...
        = buildBlock(hObject, eventdata, handles);
    
    % Wait for the bird to initiate a trial
    while handles.startOrStop == 1,
        pinCheck = 1;                                                      % Added by Efe Soyman.
...
%         pinCheck = keyStatus.key1;
        ...
        if pinCheck < 0.5 && handles.startOrStop == 1,
            ...
            [t iti response reward punish respCategory tLabels] = GoNoGoTrial(handles, songPick, reinforce);
...
function [stims, nGo, nNoGo, blockSize, song, songNames, bigBlockSize, reinforceSchedule] = buildBlock(hObject, eventdata, handles)
...
[stims, nGo, nNoGo, blockSize, song, songNames, bigBlockSize, reinforceSchedule] = buildBlock_Catch_Reinforce(hObject, eventdata, handles);
...
set(handles.nGoSongsDisplay,'String',num2str(nGo));
set(handles.nNoGoSongsDisplay,'String',num2str(nNoGo));
guidata(hObject, handles);
...
handles.preResponseDur = str2num(get(handles.preResponseDurationEdit,'String'));
...
set(handles.nGoHitsDisplay,'String',num2str(handles.responseCount(1)));
set(handles.nGoMissDisplay,'String',num2str(handles.responseCount(2)));
set(handles.nNoGoCRDisplay,'String',num2str(handles.responseCount(3)));
set(handles.nNoGoFADisplay,'String',num2str(handles.responseCount(4)));

%Keep track of # of trials reinforced
if respCategory == 1 && reward == 1,
...
function [t iti response reward punish respCategory tLabels] = GoNoGoTrial(handles, songPick, reinforce)
...
%MING END
%----------------------------
% Wait for a pre-response duration before allowing response
%----------------------------
pause(handles.preResponseDur)

%----------------------------
% Monitor the response and the playback status
%----------------------------
response = 0;                                                              % This whole part is deleted by Efe Soyman. There is a complete copy
% named GoNoGo_Copy.
...
%         pinCheck = keyStatus.key1;
        ...
        if pinCheck < 0.5, %NOTE: 8/21/08 - changed logic to normally high, look for transition to low!!
            response = 1;
            break
        end
    end
...
%----------------------------
% Reward or punish only a fraction of trials, based on
% handles.reinforceScheduleReward (and handles.reinforceSchedulePunish)
%----------------------------
if reinforce ~= 1,
    reward = 0;
    punish = 0;
...
handles.trialNum = 1;
handles.trialNumLocal = 1;
handles.responseCount(1:6) = [0 0 0 0 0 0];
handles.fractionReinforced(1:6) = [0 0 0 0 0 0];
handles.responseCountReinforced(1:6) = [0 0 0 0 0 0];
handles.nRepeats = 0;
...
saved.preResponseDur = str2num(get(handles.preResponseDurationEdit,'String'));
...
set(handles.preResponseDurationEdit,'String',num2str(saved.preResponseDur));
...
handles.preResponseDur = saved.preResponseDur;
...
function varargout = GoNoGo_OutputFcn(hObject, eventdata, handles)
